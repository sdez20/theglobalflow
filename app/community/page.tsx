...
const getAvColor = (i: number) => avColors[i % avColors.length];
...
export default function Community() {
  ...
  const [networkPosts, setNetworkPosts] = useState<any[]>([]);
  ...
  const [sarahQuestions, setSarahQuestions] = useState<any[]>([]);
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setMember(m);
      setForm({ firstName: m.first_name, lastName: m.last_name, email: m.email, password: "", linkedin: m.linkedin || "", title: m.title || "" });
      setView("hub");
    }
  }, []);

  useEffect(() => {
    if (view === "hub") { loadMessages(); loadNetworkPosts(); loadSarahQuestions(); }
  }, [view, tab]);
  ...
  async function loadSarahQuestions() {
    ...
    if (data) setSarahQuestions(data);
  }
  ...
  async function postMessage() {
    ...
    await supabase.from("messages").insert({ member_id: member.id, content: newMessage });
    setNewMessage(""); loadMessages();
  }
  async function postNetworkItem() {
    if (!newNetworkTitle.trim() || !newNetworkPost.trim() || !member) return;
    await supabase.from("network_posts").insert({ member_id: member.id, title: newNetworkTitle, content: newNetworkPost, post_type: newNetworkType, location: "" });
    setNewNetworkTitle(""); setNewNetworkPost(""); loadNetworkPosts();
  }
  ...
  async function likeMessage(id: string, current: number) { await supabase.from("messages").update({ likes: current + 1 }).eq("id", id); loadMessages(); }
  async function likeNetwork(id: string, current: number) { await supabase.from("network_posts").update({ likes: current + 1 }).eq("id", id); loadNetworkPosts(); }
  async function likeSarah(id: string, current: number) { await supabase.from("ask_sarah").update({ likes: current + 1 }).eq("id", id); loadSarahQuestions(); }
    ...
    <div style={{ fontFamily: "'Montserrat',sans-serif", color: C.charcoal, background: C.bg, minHeight: "100vh" }}>
      ...
      <nav style={{ borderBottom: `1px solid ${C.border}`, padding: "0 clamp(20px,5vw,60px)", position: "relative" }}>
        <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", height: 68 }}>
          <div style={{ position: "relative" }}>
            ...
            {dropdown && (<div style={{ position: "absolute", top: 44, left: 0, background: C.bg, border: `1px solid ${C.borderLight}`, padding: "16px 28px", zIndex: 100, minWidth: 180 }}>{[["/ ","Home"],["/#about","About"],["/#get","For You"],["/community","Community"],["/#join","Join"],["/blog","Blog"]].map(([href,l],i) => (<Link key={i} href={href.trim()} className="tracked" style={{ display: "block", padding: "10px 0", fontSize: 11, fontWeight: 500, color: C.fawn, borderBottom: i < 5 ? `1px solid ${C.border}` : "none" }}>{l}</Link>))}</div>)}
          </div>
          {view === "signup" ? (
      ...
      {view === "signup" && (<>
        ...
        <section style={{ padding: "0 clamp(20px,5vw,60px) 100px" }}>
          <form onSubmit={handleSignup} style={{ maxWidth: 440, margin: "0 auto" }}>
            ...
            <input required placeholder="First Name" value={form.firstName} onChange={(e: any) => setForm({...form, firstName: e.target.value})} style={inputStyle} />
            <input required placeholder="Last Name" value={form.lastName} onChange={(e: any) => setForm({...form, lastName: e.target.value})} style={inputStyle} />
            <input required type="email" placeholder="Email Address" value={form.email} onChange={(e: any) => setForm({...form, email: e.target.value})} style={inputStyle} />
            <input required type="password" placeholder="Create Password" value={form.password} onChange={(e: any) => setForm({...form, password: e.target.value})} style={inputStyle} />
            <input placeholder="LinkedIn Profile URL" value={form.linkedin} onChange={(e: any) => setForm({...form, linkedin: e.target.value})} style={inputStyle} />
            <input placeholder="Your Title (EA, CoS, PA, etc.)" value={form.title} onChange={(e: any) => setForm({...form, title: e.target.value})} style={inputStyle} />
      ...
      {view === "hub" && (<>
        ...
        {tab === "chat" && (<section style={{ padding: "40px clamp(20px,5vw,60px) 80px" }}><div style={{ maxWidth: 700, margin: "0 auto" }}>
          ...
          {messages.map((m: any, idx: number) => (<div key={m.id} className="msg"><div className="av" style={{ background: getAvColor(idx), color: getAvColor(idx) === "#E7B88D" ? C.fawn : C.bg }}>{m.members?.first_name?.[0]?.toUpperCase() || ""}{m.members?.last_name?.[0]?.toUpperCase() || ""}</div><div><div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, flexWrap: "wrap" as const }}><span style={{ fontSize: 14, fontWeight: 600, color: C.dark }}>{m.members?.first_name} {m.members?.last_name?.charAt(0)}.</span><span style={{ fontSize: 11, color: C.sage }}>{m.members?.title || ""}</span><span style={{ fontSize: 11, color: C.teak }}>{timeAgo(m.created_at)}</span></div><p style={{ fontSize: 14, lineHeight: 1.7, color: C.fawn, fontWeight: 300 }}>{m.content}</p><div style={{ display: "flex", gap: 16, marginTop: 10 }}><span onClick={() => likeMessage(m.id, m.likes)} style={{ fontSize: 12, color: C.sage, cursor: "pointer" }}>♡ {m.likes || 0}</span><span style={{ fontSize: 12, color: C.sage, cursor: "pointer" }}>Reply</span></div></div></div>))}
        </div></section>)}

        {tab === "network" && (<section style={{ padding: "40px clamp(20px,5vw,60px) 80px" }}><div style={{ maxWidth: 700, margin: "0 auto" }}>
          <div style={{ background: C.bone, borderRadius: 12, padding: "20px 24px", marginBottom: 16 }}><div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}><div className="av" style={{ background: C.sand, color: C.fawn }}>{initials}</div><div style={{ flex: 1 }}><input value={newNetworkTitle} onChange={(e: any) => setNewNetworkTitle(e.target.value)} placeholder="Title (e.g. EA to CEO — FinTech Startup, NYC)" style={{ width: "100%", padding: "10px 16px", marginBottom: 8, background: C.bg, border: `1px solid ${C.borderLight}`, borderRadius: 8, fontSize: 14, color: C.dark, outline: "none", fontFamily: "'Montserrat',sans-serif" }} /><textarea value={newNetworkPost} onChange={(e: any) => setNewNetworkPost(e.target.value)} placeholder="Share details about the opportunity, event, or meetup..." style={{ width: "100%", minHeight: 60, padding: "12px 16px", background: C.bg, border: `1px solid ${C.borderLight}`, borderRadius: 8, fontSize: 14, color: C.dark, outline: "none", fontFamily: "'Montserrat',sans-serif", resize: "none" }} /><div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}><div style={{ display: "flex", gap: 8 }}>{["Job", "Event", "Meetup"].map((t) => (<span key={t} onClick={() => setNewNetworkType(t)} style={{ fontSize: 10, fontWeight: 500, letterSpacing: "0.15em", textTransform: "uppercase" as const, padding: "5px 12px", borderRadius: 50, border: newNetworkType === t ? `1px solid ${C.fawn}` : `1px solid ${C.borderLight}`, background: newNetworkType === t ? C.fawn : "transparent", color: newNetworkType === t ? C.bg : C.sage, cursor: "pointer" }}>{t}</span>))}</div><button onClick={postNetworkItem} className="btn tracked" style={{ background: C.fawn, color: C.bg, border: "none", padding: "10px 24px", fontSize: 10, borderRadius: 50, cursor: "pointer", fontFamily: "'Montserrat',sans-serif" }}>Post</button></div></div></div></div>
          <div style={{ display: "flex", gap: 8, marginBottom: 28, flexWrap: "wrap" }}>{["All", "Job", "Event", "Meetup"].map((t) => (<span key={t} onClick={() => setNetworkFilter(t)} style={{ fontSize: 10, fontWeight: 500, letterSpacing: "0.2em", textTransform: "uppercase" as const, padding: "6px 16px", borderRadius: 50, background: networkFilter === t ? C.fawn : "transparent", color: networkFilter === t ? C.bg : C.sage, border: networkFilter === t ? "none" : `1px solid ${C.borderLight}`, cursor: "pointer" }}>{t === "All" ? "All" : `${t}s`}</span>))}</div>
          {networkPosts.filter((p: any) => networkFilter === "All" || p.post_type === networkFilter).length === 0 && <p style={{ textAlign: "center", color: C.sage, fontSize: 14, padding: "40px 0" }}>No posts yet. Share the first opportunity!</p>}
          {networkPosts.filter((p: any) => networkFilter === "All" || p.post_type === networkFilter).map((p: any, idx: number) => (<div key={p.id} className="post-card"><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}><div style={{ display: "flex", gap: 12, alignItems: "center" }}><div className="av" style={{ background: getAvColor(idx), color: getAvColor(idx) === "#E7B88D" ? C.fawn : C.bg }}>{p.members?.first_name?.[0]?.toUpperCase() || ""}{p.members?.last_name?.[0]?.toUpperCase() || ""}</div><div><span style={{ fontSize: 14, fontWeight: 600, color: C.dark }}>{p.members?.first_name} {p.members?.last_name?.charAt(0)}.</span><p style={{ fontSize: 11, color: C.teak, marginTop: 2 }}>{timeAgo(p.created_at)}</p></div></div><span style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase" as const, padding: "4px 12px", borderRadius: 50, background: p.post_type === "Job" ? "rgba(176,137,104,0.15)" : p.post_type === "Event" ? "rgba(231,184,141,0.2)" : "rgba(136,133,106,0.15)", color: p.post_type === "Job" ? C.teak : p.post_type === "Event" ? "#C49A6C" : C.sage }}>{p.post_type}</span></div><h3 style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 20, fontWeight: 500, color: C.dark, marginBottom: 8, lineHeight: 1.3 }}>{p.title}</h3><p style={{ fontSize: 13, lineHeight: 1.7, color: C.fawn, fontWeight: 300 }}>{p.content}</p><div style={{ display: "flex", gap: 16, marginTop: 14 }}><span onClick={() => likeNetwork(p.id, p.likes)} style={{ fontSize: 12, color: C.sage, cursor: "pointer" }}>♡ {p.likes || 0}</span><span style={{ fontSize: 12, color: C.sage, cursor: "pointer" }}>Reply</span></div></div>))}
        </div></section>)}
        ...
        {tab === "sarah" && (<section style={{ padding: "40px clamp(20px,5vw,60px) 80px" }}><div style={{ maxWidth: 700, margin: "0 auto" }}>
          ...
          {sarahQuestions.map((q: any, idx: number) => (<div key={q.id} className="msg"><div className="av" style={{ background: getAvColor(idx), color: getAvColor(idx) === "#E7B88D" ? C.fawn : C.bg }}>{q.members?.first_name?.[0]?.toUpperCase() || ""}{q.members?.last_name?.[0]?.toUpperCase() || ""}</div><div style={{ flex: 1 }}><div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, flexWrap: "wrap" as const }}><span style={{ fontSize: 14, fontWeight: 600, color: C.dark }}>{q.members?.first_name} {q.members?.last_name?.charAt(0)}.</span><span style={{ fontSize: 11, color: C.teak }}>{timeAgo(q.created_at)}</span></div><p style={{ fontSize: 14, lineHeight: 1.7, color: C.fawn, fontWeight: 300 }}>{q.question}</p>{q.sarah_reply && (<div style={{ marginTop: 16, padding: "16px 20px", background: C.bone, borderRadius: 10, borderLeft: `3px solid ${C.teak}` }}><div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}><div style={{ width: 28, height: 28, borderRadius: "50%", background: `linear-gradient(135deg, ${C.sand}, ${C.teak})`, display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 12, color: C.bg }}>S</span></div><span style={{ fontSize: 13, fontWeight: 600, color: C.dark }}>Sarah DeSouza</span></div><p style={{ fontSize: 14, lineHeight: 1.7, color: C.fawn, fontWeight: 300 }}>{q.sarah_reply}</p></div>)}<div style={{ display: "flex", gap: 16, marginTop: 10 }}><span onClick={() => likeSarah(q.id, q.likes)} style={{ fontSize: 12, color: C.sage, cursor: "pointer" }}>♡ {q.likes || 0}</span><span style={{ fontSize: 12, color: C.sage, cursor: "pointer" }}>Reply</span></div></div></div>))}
        </div></section>)}
