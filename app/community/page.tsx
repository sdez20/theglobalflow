...
function ReplyInput({ parentType, parentId, onPosted }) {
  ...
  async function submit() {
    ...
    await supabase.from("replies").insert({ member_id: member.id, parent_type: parentType, parent_id: parentId, content: text });
    setText("");
    onPosted();
  }

  return (
    <div style={{ marginLeft: 56, marginTop: 10, display: "flex", gap: 8, alignItems: "flex-end" }}>
      <textarea ref={ref} value={text} onChange={(e) => setText(e.target.value)} placeholder="Write a reply..." style={{ flex: 1, padding: "10px 14px", minHeight: 50, background: C.bone, border: "1px solid " + C.borderLight, borderRadius: 8, fontSize: 13, color: C.dark, outline: "none", fontFamily: "'Montserrat',sans-serif", resize: "none" }} />
      <button onClick={submit} style={{ padding: "10px 16px", background: C.fawn, color: C.bg, border: "none", borderRadius: 8, fontSize: 10, fontWeight: 500, cursor: "pointer", fontFamily: "'Montserrat',sans-serif" }}>Reply</button>
    </div>
  );
}

function LinkButton({ onInsert }) {
  const [show, setShow] = useState(false);
  const [url, setUrl] = useState("");

  function handleInsert() {
    if (url.trim()) { onInsert(url.trim()); setUrl(""); setShow(false); }
  }

  return (
    <div style={{ position: "relative", display: "inline-block" }}>
      <button onClick={() => setShow(!show)} style={{ padding: "6px 14px", background: "transparent", border: "1px solid " + C.borderLight, borderRadius: 50, fontSize: 11, color: C.sage, cursor: "pointer", fontFamily: "'Montserrat',sans-serif", display: "flex", alignItems: "center", gap: 6 }}>
        <span style={{ fontSize: 14 }}>🔗</span> Add Link
      </button>
      {show && (
        <div style={{ position: "absolute", bottom: 40, left: 0, background: C.bg, border: "1px solid " + C.borderLight, borderRadius: 8, padding: 12, zIndex: 50, width: 280, boxShadow: "0 4px 12px rgba(0,0,0,0.08)" }}>
          <input value={url} onChange={(e) => setUrl(e.target.value)} placeholder="Paste URL here..." style={{ width: "100%", padding: "10px 12px", background: C.bone, border: "1px solid " + C.borderLight, borderRadius: 6, fontSize: 13, color: C.dark, outline: "none", fontFamily: "'Montserrat',sans-serif", marginBottom: 8 }} />
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end" }}>
            <button onClick={() => { setShow(false); setUrl(""); }} style={{ padding: "6px 12px", background: "transparent", border: "none", fontSize: 11, color: C.sage, cursor: "pointer", fontFamily: "'Montserrat',sans-serif" }}>Cancel</button>
            <button onClick={handleInsert} style={{ padding: "6px 14px", background: C.fawn, color: C.bg, border: "none", borderRadius: 6, fontSize: 11, fontWeight: 500, cursor: "pointer", fontFamily: "'Montserrat',sans-serif" }}>Insert</button>
          </div>
        </div>
      )}
    </div>
...
export default function Community() {
  ...
  function toggleReply(key) { setReplyOpen(replyOpen === key ? null : key); }

  function renderReplies(parentType, parentId) {
    ...
    return (<>
      ...
      {replyOpen === key && <ReplyInput parentType={parentType} parentId={parentId} onPosted={() => { setReplyOpen(null); loadReplies(parentType, parentId); }} />}
    </>);
  }
      ...
      {view === "hub" && (<>
        ...
        {tab === "chat" && (<section style={{ padding: "40px clamp(20px,5vw,60px) 80px" }}><div style={{ maxWidth: 700, margin: "0 auto" }}>
          <div style={{ background: C.bone, borderRadius: 12, padding: "20px 24px", marginBottom: 32 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              <div className="av" style={{ background: C.sand, color: C.fawn }}>{initials}</div>
              <div style={{ flex: 1 }}>
                <textarea value={newMessage} onChange={(e) => setNewMessage(e.target.value)} placeholder="Share something with the community..." style={{ width: "100%", minHeight: 60, padding: "12px 16px", background: C.bg, border: "1px solid " + C.borderLight, borderRadius: 8, fontSize: 14, color: C.dark, outline: "none", fontFamily: "'Montserrat',sans-serif", resize: "none" }} />
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10 }}>
                  <LinkButton onInsert={(url) => setNewMessage(prev => prev + (prev ? " " : "") + url)} />
                  <button onClick={postMessage} className="btn tracked" style={{ background: C.fawn, color: C.bg, border: "none", padding: "10px 24px", fontSize: 10, borderRadius: 50, cursor: "pointer", fontFamily: "'Montserrat',sans-serif" }}>Post</button>
                </div>
              </div>
            </div>
          </div>
          ...
          {messages.map((m, idx) => (
            <div key={m.id}>
              <div className="msg">
                <div className="av" style={{ background: getAvColor(idx), color: getAvColor(idx) === "#E7B88D" ? C.fawn : C.bg }}>{(m.members?.first_name?.[0] || "").toUpperCase()}{(m.members?.last_name?.[0] || "").toUpperCase()}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 14, fontWeight: 600, color: C.dark }}>{m.members?.first_name} {m.members?.last_name?.charAt(0)}.</span>
                    <span style={{ fontSize: 11, color: C.sage }}>{m.members?.title || ""}</span>
                    <span style={{ fontSize: 11, color: C.teak }}>{timeAgo(m.created_at)}</span>
                  </div>
                  <p style={{ fontSize: 14, lineHeight: 1.7, color: C.fawn, fontWeight: 300 }}><Linkify text={m.content} /></p>
                  <div style={{ display: "flex", gap: 16, marginTop: 10 }}>
                    <span onClick={() => likeMessage(m.id, m.likes)} style={{ fontSize: 12, color: C.sage, cursor: "pointer" }}>♡ {m.likes || 0}</span>
                    <span onClick={() => toggleReply("message-" + m.id)} style={{ fontSize: 12, color: replyOpen === "message-" + m.id ? C.teak : C.sage, cursor: "pointer", fontWeight: replyOpen === "message-" + m.id ? 600 : 400 }}>Reply {(replies["message-" + m.id] || []).length > 0 ? "(" + (replies["message-" + m.id] || []).length + ")" : ""}</span>
                  </div>
                  {renderReplies("message", m.id)}
                </div>
              </div>
            </div>
          ))}
        ...
        {tab === "network" && (<section style={{ padding: "40px clamp(20px,5vw,60px) 80px" }}><div style={{ maxWidth: 700, margin: "0 auto" }}>
          <div style={{ background: C.bone, borderRadius: 12, padding: "20px 24px", marginBottom: 16 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              <div className="av" style={{ background: C.sand, color: C.fawn }}>{initials}</div>
              <div style={{ flex: 1 }}>
                <input value={newNetworkTitle} onChange={(e) => setNewNetworkTitle(e.target.value)} placeholder="Title (e.g. EA to CEO — FinTech Startup, NYC)" style={{ width: "100%", padding: "10px 16px", marginBottom: 8, background: C.bg, border: "1px solid " + C.borderLight, borderRadius: 8, fontSize: 14, color: C.dark, outline: "none", fontFamily: "'Montserrat',sans-serif" }} />
                <textarea value={newNetworkPost} onChange={(e) => setNewNetworkPost(e.target.value)} placeholder="Share details about the opportunity, event, or meetup..." style={{ width: "100%", minHeight: 60, padding: "12px 16px", background: C.bg, border: "1px solid " + C.borderLight, borderRadius: 8, fontSize: 14, color: C.dark, outline: "none", fontFamily: "'Montserrat',sans-serif", resize: "none" }} />
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 10, flexWrap: "wrap", gap: 10 }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                    {["Job", "Event", "Meetup"].map((t) => (<span key={t} onClick={() => setNewNetworkType(t)} style={{ fontSize: 10, fontWeight: 500, letterSpacing: "0.15em", textTransform: "uppercase", padding: "5px 12px", borderRadius: 50, border: newNetworkType === t ? "1px solid " + C.fawn : "1px solid " + C.borderLight, background: newNetworkType === t ? C.fawn : "transparent", color: newNetworkType === t ? C.bg : C.sage, cursor: "pointer" }}>{t}</span>))}
                    <LinkButton onInsert={(url) => setNewNetworkPost(prev => prev + (prev ? " " : "") + url)} />
                  </div>
                  <button onClick={postNetworkItem} className="btn tracked" style={{ background: C.fawn, color: C.bg, border: "none", padding: "10px 24px", fontSize: 10, borderRadius: 50, cursor: "pointer", fontFamily: "'Montserrat',sans-serif" }}>Post</button>
                </div>
              </div>
            </div>
          </div>
          ...
          {networkPosts.filter(p => networkFilter === "All" || p.post_type === networkFilter).map((p, idx) => (
            <div key={p.id} className="post-card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 14 }}>
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  <div className="av" style={{ background: getAvColor(idx), color: getAvColor(idx) === "#E7B88D" ? C.fawn : C.bg }}>{(p.members?.first_name?.[0] || "").toUpperCase()}{(p.members?.last_name?.[0] || "").toUpperCase()}</div>
                  <div><span style={{ fontSize: 14, fontWeight: 600, color: C.dark }}>{p.members?.first_name} {p.members?.last_name?.charAt(0)}.</span><p style={{ fontSize: 11, color: C.teak, marginTop: 2 }}>{timeAgo(p.created_at)}</p></div>
                </div>
                <span style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.15em", textTransform: "uppercase", padding: "4px 12px", borderRadius: 50, background: p.post_type === "Job" ? "rgba(176,137,104,0.15)" : p.post_type === "Event" ? "rgba(231,184,141,0.2)" : "rgba(136,133,106,0.15)", color: p.post_type === "Job" ? C.teak : p.post_type === "Event" ? "#C49A6C" : C.sage }}>{p.post_type}</span>
              </div>
              ...
              <p style={{ fontSize: 13, lineHeight: 1.7, color: C.fawn, fontWeight: 300 }}><Linkify text={p.content} /></p>
              <div style={{ display: "flex", gap: 16, marginTop: 14 }}>
                <span onClick={() => likeNetwork(p.id, p.likes)} style={{ fontSize: 12, color: C.sage, cursor: "pointer" }}>♡ {p.likes || 0}</span>
                <span onClick={() => toggleReply("network-" + p.id)} style={{ fontSize: 12, color: replyOpen === "network-" + p.id ? C.teak : C.sage, cursor: "pointer", fontWeight: replyOpen === "network-" + p.id ? 600 : 400 }}>Reply {(replies["network-" + p.id] || []).length > 0 ? "(" + (replies["network-" + p.id] || []).length + ")" : ""}</span>
              </div>
              {renderReplies("network", p.id)}
            </div>
          ))}
        </div></section>)}
        ...
        {tab === "sarah" && (<section style={{ padding: "40px clamp(20px,5vw,60px) 80px" }}><div style={{ maxWidth: 700, margin: "0 auto" }}>
          <div style={{ background: C.bone, borderRadius: 12, padding: 32, textAlign: "center", marginBottom: 32 }}>
            <div style={{ width: 64, height: 64, borderRadius: "50%", background: "linear-gradient(135deg, " + C.sand + ", " + C.teak + ")", margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 26, color: C.bg, fontWeight: 300 }}>S</span></div>
          ...
          <div style={{ marginBottom: 32 }}>
            <textarea value={newQuestion} onChange={(e) => setNewQuestion(e.target.value)} placeholder="What's on your mind?" style={{ width: "100%", minHeight: 80, padding: "16px 20px", background: C.bone, border: "1px solid " + C.borderLight, borderRadius: 8, fontSize: 14, color: C.dark, outline: "none", fontFamily: "'Montserrat',sans-serif", resize: "none" }} />
            <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 10 }}>
              <button onClick={askSarah} className="btn tracked" style={{ background: C.fawn, color: C.bg, border: "none", padding: "10px 24px", fontSize: 10, borderRadius: 50, cursor: "pointer", fontFamily: "'Montserrat',sans-serif" }}>Ask Sarah</button>
            </div>
          </div>
          ...
          {sarahQuestions.map((q, idx) => (
            <div key={q.id}>
              <div className="msg">
                <div className="av" style={{ background: getAvColor(idx), color: getAvColor(idx) === "#E7B88D" ? C.fawn : C.bg }}>{(q.members?.first_name?.[0] || "").toUpperCase()}{(q.members?.last_name?.[0] || "").toUpperCase()}</div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, flexWrap: "wrap" }}>
                    <span style={{ fontSize: 14, fontWeight: 600, color: C.dark }}>{q.members?.first_name} {q.members?.last_name?.charAt(0)}.</span>
                    <span style={{ fontSize: 11, color: C.teak }}>{timeAgo(q.created_at)}</span>
                  </div>
                  <p style={{ fontSize: 14, lineHeight: 1.7, color: C.fawn, fontWeight: 300 }}><Linkify text={q.question} /></p>
                  {q.sarah_reply && (
                    <div style={{ marginTop: 16, padding: "16px 20px", background: C.bone, borderRadius: 10, borderLeft: "3px solid " + C.teak }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                        <div style={{ width: 28, height: 28, borderRadius: "50%", background: "linear-gradient(135deg, " + C.sand + ", " + C.teak + ")", display: "flex", alignItems: "center", justifyContent: "center" }}><span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: 12, color: C.bg }}>S</span></div>
                        <span style={{ fontSize: 13, fontWeight: 600, color: C.dark }}>Sarah DeSouza</span>
                      </div>
                      <p style={{ fontSize: 14, lineHeight: 1.7, color: C.fawn, fontWeight: 300 }}><Linkify text={q.sarah_reply} /></p>
                    </div>
                  )}
                  <div style={{ display: "flex", gap: 16, marginTop: 10 }}>
                    <span onClick={() => likeSarah(q.id, q.likes)} style={{ fontSize: 12, color: C.sage, cursor: "pointer" }}>♡ {q.likes || 0}</span>
                    <span onClick={() => toggleReply("sarah-" + q.id)} style={{ fontSize: 12, color: replyOpen === "sarah-" + q.id ? C.teak : C.sage, cursor: "pointer", fontWeight: replyOpen === "sarah-" + q.id ? 600 : 400 }}>Reply {(replies["sarah-" + q.id] || []).length > 0 ? "(" + (replies["sarah-" + q.id] || []).length + ")" : ""}</span>
                  </div>
                  {renderReplies("sarah", q.id)}
                </div>
              </div>
            </div>
          ))}
